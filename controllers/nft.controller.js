...
const create = async (req, res) => {
  ...
  if (nftCollection) {
    ...
    if (collectionName) {
      ...
      data = await Nft.findByIdAndUpdate(data._id, { collectionId: col._id });
    }
...
const getallCollections = async (req, res) => {
  ...
  if (collections.length < skip + 30) {
    ...
    const data = await Collection.find({ storefront }).limit(limit).skip(skip);
    res.status(StatusCodes.OK).json({
      data: data,
...
const getTrendingCollections = async (req, res) => {
  try {
    ...
    const nfts = Nft.aggregate([
      { $match: { collectionName: { $eq:collections[i].collectionName } } },
      { $group: { _id: null, views: { $sum: "$views" } } },
    ]);
    trendingCollection.push({"collection":collections[i], "views":nfts.views})
}
